<template>
<div class="calendar-mask" @click="closeCalendar">
    <div class="calendar" id="swipeCalendar">
...
export default {
    ...
    methods: {
        ...
        closeCalendar(e){
            ...
            if(e.target.className&&e.target.className=='calendar-mask'){
                this.$emit('select');
            }
        ...
        initSwipe(){
            var that=this;
            var div=document.getElementById('swipeCalendar');
            ...
            function touch (event){  
                ...
                switch(event.type){  
                    case "touchstart":  
                        ...
                        break;  
                    case "touchend": 
                        if(event.changedTouches[0].clientX -pagex>10){
                            that.prev();   
                        }else if(event.changedTouches[0].clientX -pagex<-10){
                            that.next();
                        }
                        break;  
                    case "touchmove":  
        ...
        select(k1, k2, e) {
            ...
            if (this.range) {
                if (this.rangeBegin.length == 0 || this.rangeEndTemp != 0) {
                    ...
                    this.events=[];//清除日期里的提示
                    var day=this.days[k1][k2].day;
                    var month=this.month+1;
                    var months=this.month+1;

                    //日期选中后此日期前不能选，超过当前月份自动增加一个月
                    if(k2==6){
                        k1++;
                        k2=0;
                        if(this.days[k1][k2].day==1){
                            month++;
                        }
                    }else{
                        k2++;
                   ...
                   if(months<10){
                    ...
                    this.begin=[this.year, month,this.days[k1][k2].day];//重新设置开始日期
                        
                    var date=this.year+'-'+months+'-'+day;
                    this.events={[date]:this.eventsName};
